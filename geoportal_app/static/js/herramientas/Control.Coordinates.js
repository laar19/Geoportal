...
L.Control.Coordinates = L.Control.extend({
	...
	onAdd: function(map)
	{
		...
		this.visible = false;
		this.active = false; // NUEVO: Estado activo
	...
	{
		// NUEVO: Solo mostrar la ventana si la herramienta está activa
		...
		if (obj.latlng) {
			...
			// --- LÓGICA PARA DIBUJAR EL PUNTO (MARCADOR) ---
			// Solo si la herramienta está activa
			if (this.active) {
				// Eliminar el marcador anterior si existe
				if (this._marker) {
					this._map.removeLayer(this._marker);
				}
				// Crear y añadir el nuevo marcador al mapa
				this._marker = L.marker(obj.latlng).addTo(this._map);
			}
			// ---------------------------------------------
		}
	}
